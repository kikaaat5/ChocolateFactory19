...
function loadChocolate(){
    axios.get('http://localhost:8080/WebShopAppREST/rest/chocolates/' + id.value).then(response=>{
        if(response.data!=""){
            c=response.data;
        }else{
            alert('Greska pri dobavljanju cokolade');
        }        
    });
}
...
function saveChocolate(event){
    ...
    axios.put('http://localhost:8080/WebShopAppREST/rest/chocolates/' + id.value, this.chocolate).then(response=>{
        push.back();
    });
}
